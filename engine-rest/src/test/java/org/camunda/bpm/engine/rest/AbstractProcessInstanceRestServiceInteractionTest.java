...
public abstract class AbstractProcessInstanceRestServiceInteractionTest extends
  ...
  public void testBulkCommandsDelete() {
	...
	commandVariablesJson.put("deleteReason", MockProvider.EXAMPLE_HISTORIC_PROCESS_INSTANCE_DELETE_REASON); 
	...
	given().contentType(ContentType.JSON).body(commandDetailsJson)
	 ...
	 verify(runtimeServiceMock).deleteProcessInstance(MockProvider.EXAMPLE_PROCESS_INSTANCE_ID, MockProvider.EXAMPLE_HISTORIC_PROCESS_INSTANCE_DELETE_REASON);
	 verify(runtimeServiceMock).deleteProcessInstance(MockProvider.ANOTHER_EXAMPLE_PROCESS_INSTANCE_ID, MockProvider.EXAMPLE_HISTORIC_PROCESS_INSTANCE_DELETE_REASON);
  }
  ...
  public void testBulkCommandsDeleteNonExistingId() {
	...
	doThrow(new ProcessEngineException(exceptionMessage))
	      .when(runtimeServiceMock).deleteProcessInstance(eq(MockProvider.EXAMPLE_NON_EXISTENT_PROCESS_INSTANCE_ID), eq(MockProvider.EXAMPLE_HISTORIC_PROCESS_INSTANCE_DELETE_REASON));
	  
	doThrow(new ProcessEngineException(anotherExceptionMessage))
        .when(runtimeServiceMock).deleteProcessInstance(eq(MockProvider.EXAMPLE_ANOTHER_NON_EXISTENT_PROCESS_INSTANCE_ID), eq(MockProvider.EXAMPLE_HISTORIC_PROCESS_INSTANCE_DELETE_REASON));
